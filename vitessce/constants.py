...
class CoordinationType(DocEnum):
    ...
    META_COORDINATION_SCOPES_BY = (
        ...
        "Shared representation of view-level coordinationScopesBy.",
    )
    ...
    OBS_LABELS_TYPE = (
        "obsLabelsType",
        "Feature for displaying additional obs sets' data in heatmap/scatterplot/spatial tooltips.",
    )
    ...
    EMBEDDING_OBS_SET_POLYGONS_VISIBLE = (
        "embeddingObsSetPolygonsVisible",
        "Whether polygon boundaries for each selected obsSet are visible in the embedding scatterplt.",
    )
    EMBEDDING_OBS_SET_LABELS_VISIBLE = (
        "embeddingObsSetLabelsVisible",
        "Whether labels for each selected obsSet are visible in the embedding scatterplot.",
    )
    EMBEDDING_OBS_SET_LABEL_SIZE = (
        "embeddingObsSetLabelSize",
        "The size of labels for selected obsSets in the embedding scatterplot.",
    )
    ...
    EMBEDDING_OBS_RADIUS_MODE = (
        "embeddingObsRadiusMode",
        "Radius mode of cells in embedding (points in scatterplot view) - auto or manual",
    )
    EMBEDDING_OBS_OPACITY_MODE = (
        "embeddingObsOpacityMode",
        "Opacity mode of cells in embedding (points in scatterplot view) - auto or manual",
    )
    ...
    SPATIAL_SEGMENTATION_LAYER = (
        "spatialSegmentationLayer",
        "Layer definitions for the segmentations in the spatial view.",
    )
    SPATIAL_POINT_LAYER = "spatialPointLayer", "Layer definitions for the points in the spatial view."
    SPATIAL_NEIGHBORHOOD_LAYER = (
        "spatialNeighborhoodLayer",
        "Layer definitions for the neighborhoods in the spatial view.",
    )
    ...
    FEATURE_VALUE_TRANSFORM_COEFFICIENT = (
        ...
        "Coefficient to transform values in the gating scatterplot.",
    )
    TOOLTIPS_VISIBLE = (
        "tooltipsVisible",
        "Boolean for whether or not tooltips are visible, used by the scatterplot, spatial, and heatmap views.",
    )
...
class ViewType(DocEnum):
    ...
    SPATIAL = (
        "spatial",
        "The spatial component can be used for visualization of cells, molecules, or images in spatial coordinates.",
    )
    ...
    LAYER_CONTROLLER = (
        "layerController",
        "The layer controller can be used to manipulate channel settings of the images rendered by the spatial component.",
    )
    GENOMIC_PROFILES = (
        "genomicProfiles",
        "The higlass component can be used to visualize genome-wide ATAC-seq profiles.",
    )
    OBS_SETS = "obsSets", "Observation sets"
    OBS_SET_SIZES = "obsSetSizes", "Observation set sizes bar plot"
    OBS_SET_FEATURE_VALUE_DISTRIBUTION = (
        "obsSetFeatureValueDistribution",
        "Violin plot visualizing the distribution of feature values per observation set.",
    )
    FEATURE_VALUE_HISTOGRAM = (
        "featureValueHistogram",
        "Histogram visualizing the distribution of values for a selected feature across all observations.",
    )
...
class FileType(DocEnum):
    ...
    OBS_FEATURE_MATRIX_ANNDATA_ZARR = (
        "obsFeatureMatrix.anndata.zarr",
        "File type for obsFeatureMatrix stored in an AnnData object saved to a Zarr store",
    )
    OBS_SETS_ANNDATA_ZARR = (
        "obsSets.anndata.zarr",
        "File type for obsSets stored in an AnnData object saved to a Zarr store",
    )
    OBS_EMBEDDING_ANNDATA_ZARR = (
        "obsEmbedding.anndata.zarr",
        "File type for obsEmbedding values stored in an AnnData object saved to a Zarr store",
    )
    OBS_LOCATIONS_ANNDATA_ZARR = (
        "obsLocations.anndata.zarr",
        "File type for obsLocations values stored in an AnnData object saved to a Zarr store",
    )
    OBS_SEGMENTATIONS_ANNDATA_ZARR = (
        "obsSegmentations.anndata.zarr",
        "File type for obsSegmentations polygons stored in an AnnData object saved to a Zarr store",
    )
    OBS_LABELS_ANNDATA_ZARR = (
        "obsLabels.anndata.zarr",
        "File type for obsLabels stored in an AnnData object saved to a Zarr store",
    )
    FEATURE_LABELS_ANNDATA_ZARR = (
        "featureLabels.anndata.zarr",
        "File type for featureLabels stored in an AnnData object saved to a Zarr store",
    )
    ...
    ANNDATA_EXPRESSION_MATRIX_ZARR = (
        "anndata-expression-matrix.zarr",
        "The Zarr-based expression matrix file type from an anndata object.",
    )
