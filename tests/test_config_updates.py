...
def test_get_view_by_index(vitessce_config):
    view = vitessce_config.get_view_by(0)
    vc_dict = view.to_dict()
    assert vc_dict["component"] == "spatial"

    view = vitessce_config.get_view_by(1)
    vc_dict = view.to_dict()
    assert vc_dict["component"] == "scatterplot"

    with pytest.raises(IndexError):
        vitessce_config.get_view_by(5)


def test_get_view_by_component(vitessce_config):
    view = vitessce_config.get_view_by("spatial")
    vc_dict = view.to_dict()
    assert vc_dict["component"] == "spatial"

    view = vitessce_config.get_view_by("SCATTERPLOT")
    vc_dict = view.to_dict()
    assert vc_dict["component"] == "scatterplot"

    with pytest.raises(ValueError):
        vitessce_config.get_view_by("TEST")


def test_get_view_by_invalid_type(vitessce_config):
    with pytest.raises(TypeError):
        vitessce_config.get_view_by(3.5)


def test_remove_view_by_index(vitessce_config):
    removed_view = vitessce_config.remove_view_by(0)
    rv_dict = removed_view.to_dict()
    assert rv_dict["component"] == "spatial"
    assert len(vitessce_config.get_views()) == 2

    removed_view = vitessce_config.remove_view_by(1)
    rv_dict = removed_view.to_dict()
    assert rv_dict["component"] == "scatterplot"
    assert len(vitessce_config.get_views()) == 1

    with pytest.raises(IndexError):
        vitessce_config.remove_view_by(5)


def test_remove_view_by_component(vitessce_config):
    removed_view = vitessce_config.remove_view_by("spatial")
    rv_dict = removed_view.to_dict()
    assert rv_dict["component"] == "spatial"
    assert len(vitessce_config.get_views()) == 2

    with pytest.raises(ValueError):
        vitessce_config.remove_view_by("spatial")

    with pytest.raises(ValueError):
        vitessce_config.remove_view_by("TEST")


def test_remove_view_by_invalid_type(vitessce_config):
    with pytest.raises(TypeError):
        vitessce_config.remove_view_by(3.5)
