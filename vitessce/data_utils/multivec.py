...
def adata_to_multivec_zarr(
    ...
    in_bins_df["chr_name"] = in_bins_df["interval"].apply(convert_bin_name_to_chr_name)
    in_bins_df["chr_start"] = in_bins_df["interval"].apply(convert_bin_name_to_chr_start)
    in_bins_df["chr_end"] = in_bins_df["interval"].apply(convert_bin_name_to_chr_end)
    ...
    # Round bins
    in_bins_df["chr_start_round"] = in_bins_df["chr_start"].apply(
    ...
    in_bins_df["interval"] = in_bins_df.apply(
    ...
    for chr_name in tqdm(chrom_names):
        ...
        chr_bins_in_join_df["i"] = chr_bins_in_join_df.apply(
            lambda r: r["i"] if pd.notna(r["i"]) else r["i_gt"], axis="columns"
        ).astype(int)
        ...
        for cluster_index, cluster_id in enumerate(cluster_ids):
            ...
            cluster_df = in_barcodes_df.loc[in_barcodes_df[obs_set_col] == cluster_id]
            cluster_cell_ids = cluster_df.index.values.tolist()
            cluster_cells_tf = (in_barcodes_df.index.to_series().isin(cluster_cell_ids)).values
            ...
            genomic_profiles.add_profile(cluster_profile[0:profile_len], chr_name, cluster_index)
